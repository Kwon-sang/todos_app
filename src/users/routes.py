...
async def retrieve_user_info(auth: AuthDependency,
    ...
    # Retrieve user info
    return DB.retrieve_one(User, id=auth.user_id)
...
async def change_password(auth: AuthDependency,
    ...
    # Retrieve User by ID
    user = DB.retrieve_one(User, id=user_id)
    ...
    DB.patch(User, target={"hashed_password": hashed_password}, id=user_id)
...
async def change_user_info(auth: AuthDependency,
    ...
    service.verify_current_user(auth.user_id, user_id)
    # Update user's info
    DB.update(User, body=body, id=user_id)
